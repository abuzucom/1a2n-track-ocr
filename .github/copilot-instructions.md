...
```
install:      cd server && pip install -r requirements.lock
              cd ml && pip install -r requirements.lock
install BLE:  cd server && pip install -r requirements-ble.lock
              (only for the BLE transport; regenerate the lock on your OS)
test all:     cd server && python -m pytest tests/
...
BLE server:   cd server && BLE_ENABLED=1 uvicorn app:app --host 127.0.0.1 --port 8000
...
```
firmware/src/charset.h      generated by ml/export_charset.py
firmware/src/ocr_model.h    generated by ml/export_model_header.py
firmware/src/config.h       gitignored, per-rig transport and credentials
CLAUDE.md and the 7 other   generated by scripts/sync.py; edit AGENTS.md
  rule copies
*.lock                      generated by pip-compile
ml/fonts/                   third-party, OFL licensed
docs/*.pdf                  vendor reference documents
```
...
A rig reaches the backend over one of two transports, chosen at build
time in `config.h`. BLE is the default and the normal operating mode;
WiFi uploads frames, so it is the mode that collects training data.

```
firmware/src/  main.cpp capture loop, camera_pins.h, char_segment.cpp,
               ondevice_ocr.cpp, uploader.cpp WiFi transport,
               ble_transport.cpp BLE transport, result_json.cpp shared body
server/        app.py HTTP entry, ocr.py Tesseract, arbiter.py source
               arbitration, sinks.py OBS output, dataset.py auto-labeling,
               ingest.py shared result path, ble_bridge.py BLE central
...
The BLE bridge is not its own process: it starts from `app.py`'s lifespan
handler, because `arbiter` and `sinks` keep process-local state that a
second process would duplicate and race on.

Public API surface (rules 4 and 5): `POST /frame`, `POST /result`, the
`now_playing.json` schema OBS overlays consume, and the BLE GATT contract
in `ble_bridge.py` and `ble_transport.cpp` (service and characteristic
UUIDs, and the frame header). A UUID constant does not look like an API,
but a flashed rig cannot be renegotiated with, so it is one.

## Gotchas

- Build firmware from PowerShell, not Git Bash; the ESP-IDF toolchain
  installer fails under MSys.
- Board must be `esp32-s3-devkitc1-n16r8`; overriding flash/PSRAM on the
  base board silently does not apply.
- `ocr_model.h`'s placeholder is a compile-time constant, so a green
  build does not prove the inference path compiles.
- Tesseract is unpinned and its output is the training set's ground truth.
- `pytesseract.image_to_boxes` uses a bottom-left origin,
...
- On `TRANSPORT_WIFI`, firmware always connects through Caddy over HTTPS
  with a pinned CA cert (`BACKEND_CA_CERT` in `config.h`); it never talks
  to uvicorn directly. Both Caddy and uvicorn must be running.
- On `TRANSPORT_BLE` there is no Caddy in the path, so the limits it
  applies to HTTP ingress do not exist. `ble_bridge.py` carries its own
  equivalents; `MAX_MESSAGE_BYTES` there stands in for the Caddyfile's
  `request_body` cap.
- `arbiter.record_ondevice` publishes nothing unless a Tesseract result
  for the same `capture_id` is pending, or the caller passes
  `sole_source=True`. This is the trap that makes a working BLE
  deployment look dead: results arrive, nothing is ever written.
- BLE builds send results only, so there are no frames, no Tesseract, and
  no dataset auto-labeling. A BLE rig contributes no training data, and
  the model it depends on can only come from a WiFi run. BLE is the
  default build, so this ordering is easy to miss.
...
```
XDJ screen layout: docs/xdj_screen_reference.md
W11 board pin map: docs/hardware_documentation.md
BLE transport:     docs/ble_transport.md
dataset format:    ml/README.md
licensing:         THIRD-PARTY-NOTICES.md
```
